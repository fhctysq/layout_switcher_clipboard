...
#include <uxtheme.h> 
#include <strsafe.h> 
#include <shellapi.h> // ДОДАНО: Для роботи з системним лотком (Shell_NotifyIcon)
...
// ==========================================
#define WM_TRAYICON (WM_APP + 2) // Кастомне повідомлення для лотка
...
HANDLE hMemHeap = NULL;               // "Пісочниця" пам'яті на 1 МБ
...
bool isCapturingAltC = false;         // Прапорець, що зараз йде обробка гарячої клавіші Alt+C
    ...
    if (nCode == HC_ACTION) {
        ...
        if (isKeyDown) {
            ...
            if (isLeftAltPressed && !isRightAltPressed) { 
                ...
                // ДОДАНО: Гаряча клавіша для виходу (Alt+Q)
...
// ==========================================
// --- РОБОТА З ФАЙЛАМИ ТА ВЕЛИКИМ ТЕКСТОМ ---
// ==========================================
...
void CustomCopyOrCut(WORD vkCode) { 
    ignoreClipboardUpdate = true; 
    BackupSysClipboard();         

    isCapturingAltC = true;
...
void UpdateListBox() {
    ...
    for (int i = 0; i < historyCount; ++i) {
        ...
        for (int k = 0; textPtr[k] != L'\0' && j < UI_PREVIEW_LENGTH; k++) {
            if (textPtr[k] == L'\r') continue; 
            if (textPtr[k] == L'\n') { display[j++] = L' '; continue; } 
            display[j++] = textPtr[k];
        }
...
LRESULT CALLBACK ListBoxSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_RBUTTONDOWN) {
        ...
        if (HIWORD(itemInfo) == 0) { 
            SendMessage(hwnd, LB_SETCURSEL, LOWORD(itemInfo), 0);
        }
        return 0; 
    }
    if (msg == WM_LBUTTONUP) { 
...
// ДОДАНО: Функція для керування іконкою в системному лотку
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: 
            ManageTrayIcon(hwnd, NIM_ADD); // ДОДАНО: Реєструємо іконку в треї при старті
        ...
        case WM_TRAYICON: // ДОДАНО: Обробка кліків по іконці в треї
        ...
        case WM_LBUTTONDOWN: { 
            ...
            if (pt.y < UI_HEADER_HEIGHT && pt.x >= UI_WIN_WIDTH - UI_HEADER_HEIGHT) {
                // ДОДАНО: Перевірка Shift для повного закриття, інакше ховаємо
        ...
        case WM_PAINT: { 
            ...
            // ДОДАНО: Текст підказки про вихід у шапці
            ...
            // ДОДАНО: Текст підказки про Shift + X знизу
        ...
        case WM_CLIPBOARDUPDATE: { 
            if (ignoreClipboardUpdate) break; 
        ...
        case MSG_PROCESS_HOTKEY: {
            ...
            else if (cmd == HotkeyCmd::Cut) { CustomCopyOrCut(0x58); }      
        ...
        case WM_CLOSE: // ДОДАНО: Явне руйнування вікна
            DestroyWindow(hwnd);
            return 0;

        case WM_DESTROY: 
            ManageTrayIcon(hwnd, NIM_DELETE); // ДОДАНО: Видаляємо іконку з трею при закритті
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    // ДОДАНО: Логіка Мутексу (заборона множинних копій і реалізація Alt+Q як перемикача)
    ...
    // ДОДАНО: Намагаємось завантажити іконку з exe для вікна, або беремо стандартну
    ...
    // ДОДАНО: Звільнення мутексу при виході
    CloseHandle(hMutex);
